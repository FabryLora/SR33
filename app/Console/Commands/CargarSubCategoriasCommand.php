<?php

namespace App\Console\Commands;

use App\Jobs\CargarCategoriasJob;
use App\Jobs\CargarSubCategoriasJob;
use Illuminate\Console\Command;
use App\Jobs\ImportarProductosDesdeExcelJob;
use Illuminate\Support\Facades\Storage;

class CargarSubCategoriasCommand extends Command
{
    protected $signature = 'subcategorias:cargar {archivo}';
    protected $description = 'Cargar subcategorías desde un archivo Excel';

    public function handle()
    {
        $archivo = $this->argument('archivo'); // ejemplo: files/parabolico.xlsx

        // Buscar en disco 'public' (storage/app/public)
        if (!Storage::disk('public')->exists($archivo)) {
            $this->error("El archivo '{$archivo}' no se encuentra en storage/app/public");
            return 1;
        }

        // Despachar usando path relativo a storage/app


        CargarSubCategoriasJob::dispatch($archivo);

        $this->info("Importación iniciada con éxito para el archivo: {$archivo}");
        return 0;
    }
}
